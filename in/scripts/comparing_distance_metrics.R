...
library(PRROC);library(ggpubr);library(distanceHD);library(stringr)
# Uniprot mapping
...
setnames(GS,c('Protein_1','Protein_2','Class'))
  ...
  list_dataset <- list(stressed_isolates = subset_stress,
...
list_PRcurves[[dataset]] <- f_BIC(df, paste(dataset,'bicor',sep = '_'))
